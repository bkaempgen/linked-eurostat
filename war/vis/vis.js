 ...
 */
function visualisiereQuery(/* JSON */ data){ 
	
	// Assignment of the xAxis, yAxis and Label variables according to the specefications. Other variables mustn't appear in the query. 
	try{
		const xAxis = data.head.vars[0];
		const yAxis = data.head.vars[1];
	...
	try{
		var time = bindings[0][xAxis].value;
	}
	catch(e){
	...
	bindings.forEach(function(d) {return d[xAxis].value = dateFormat.parse(d[xAxis].value); });
	...
	for(var i in nestedArray){
		for(var j in nestedArray[i].values){	
			nestedArray[i].values[j][yAxis].value = deleteNonNumerical(nestedArray[i].values[j][yAxis].value); 		
		}
	}
					
	// Protovis 3.2 linear scales
	var x = pv.Scale.linear(bindings, function(d) { return d[xAxis].value; } ).range(0, width-xFreiraum);
	var y = pv.Scale.linear(pv.min(bindings, function(d) { return d[yAxis].value; } ), pv.max(bindings, function(d) { return d[yAxis].value; } )).range(yFreiraum, height).nice();	
	...
	panel.add(pv.Line)
		...
		.left(function(d) { return x(d[xAxis].value); } )	
		.bottom(function(d) { return y(d[yAxis].value); } ) 
